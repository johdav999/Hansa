...
def names(v):
 if isinstance(v,dict):
  if 'name' in v and 'HansaWindmillClearanceTools' in str(v['name']):yield v['name']
...
identity=json.dumps({'project':str(P.parents[2]/'Hansa.uproject')});print(identity);ident=json.loads(identity);assert str(P.parents[2]/'Hansa.uproject').replace('\\','/').lower()==ident['project'].replace('\\','/').lower();(P/'engine_identity.json').write_text(identity)
