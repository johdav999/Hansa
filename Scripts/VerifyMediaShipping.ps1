...
try {
    ...
    Invoke-HansaNativeCommand -FilePath $context.UnrealEditorCommand -Arguments @(
        ...
        '-DisablePlugins=ModelContextProtocol,AllToolsets,GameFeatures,Water,Landmass',
...
Write-HansaJsonArtifact -Path $resultPath -Value ([ordered]@{
    ...
    AuditProcessDisabledPlugins = @('ModelContextProtocol', 'AllToolsets', 'GameFeatures', 'Water', 'Landmass')
