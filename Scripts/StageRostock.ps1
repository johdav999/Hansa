...
$artifactDirectory = New-HansaArtifactDirectory -Context $context -Operation 'p31-stage-rostock'
...
$operations = if ($Create) { @('BuildCandidate','ReviseCandidate') } else { @('ReviseCandidate') }
...
try {
    ...
    foreach ($operation in $operations) {
        ...
        Invoke-HansaNativeCommand -FilePath $context.UnrealEditorCommand -Arguments @(
            $context.ProjectFile, '-unattended', '-nop4', '-nosplash', '-NoSound', '-RenderOffscreen', '-P31Authoring',
            "-ExecCmds=Automation RunTests Hansa.World.Rostock.$operation;Quit",
            '-TestExit=Automation Test Queue Empty', "-AbsLog=$unrealLog"
        ) -LogPath (Join-Path $artifactDirectory "$operation-command.log") -FailureMessage "P31 $operation failed" | Out-Null
        ...
        if ($log -notmatch 'TEST COMPLETE\. EXIT CODE: 0' -or $log -match 'Test Completed\. Result=\{Fail') {
            throw "P31 $operation did not pass: $unrealLog"
        }
    }
} finally {
    [Environment]::SetEnvironmentVariable('UE_SKIP_UBT_SDK_SETUP',$previousSdkSetting,'Process')
}
Write-Output "P31 staged candidate saved. Production acceptance remains open. Artifacts: $artifactDirectory"
