...
namespace
{
bool StaticPropFail(FString& Error, const TCHAR* Message) { Error = Message; return false; }
FString StaticPropText(const TSharedPtr<FJsonObject>& P, const TCHAR* Key) { FString S; if (P) P->TryGetStringField(Key, S); return S; }
double StaticPropNumber(const TSharedPtr<FJsonObject>& P, const TCHAR* Key) { double N = 0; if (P) P->TryGetNumberField(Key, N); return N; }
}
bool FHansaStaticProp::ValidateProfile(const TSharedPtr<FJsonObject>& P, FString& Error)
{
    if (!P || P->Values.Num() != 11 || StaticPropNumber(P, TEXT("version")) != 1 || StaticPropText(P, TEXT("role")) != TEXT("HarborProp") ||
        !StaticPropText(P, TEXT("stableId")).StartsWith(TEXT("Prop.")) ||
        StaticPropText(P, TEXT("forwardAxis")) != TEXT("+X") || StaticPropText(P, TEXT("upAxis")) != TEXT("+Z") ||
        StaticPropText(P, TEXT("pivot")) != TEXT("bottom-center") || StaticPropText(P, TEXT("collision")) != TEXT("box"))
        return StaticPropFail(Error, TEXT("HarborProp v1 requires Prop.* identity, +X forward, +Z up, bottom-center pivot and box collision."));
    const FString StableId = StaticPropText(P, TEXT("stableId"));
    TArray<FString> IdParts;
    StableId.ParseIntoArray(IdParts, TEXT("."), false);
    if (StableId.Len() > 100 || IdParts.Num() < 2)
        return StaticPropFail(Error, TEXT("Static prop requires a bounded Prop.* stable ID."));
    for (const FString& Part : IdParts)
    {
        if (Part.IsEmpty()) return StaticPropFail(Error, TEXT("Static prop stable ID contains an empty segment."));
        for (TCHAR C : Part)
            if (!((C >= 'A' && C <= 'Z') || (C >= 'a' && C <= 'z') || (C >= '0' && C <= '9') || C == '_'))
                return StaticPropFail(Error, TEXT("Static prop stable ID contains an unsupported character."));
    }
    ...
    {
        const double N = StaticPropNumber(P, *Limit.Get<0>());
        if (!FMath::IsFinite(N) || N < Limit.Get<1>() || N > Limit.Get<2>() || N != FMath::FloorToDouble(N))
            return StaticPropFail(Error, TEXT("Static-prop profile exceeds a canonical size or resource limit."));
    }
    return true;
}
bool FHansaStaticProp::Prepare(const TArray<UObject*>& Assets, const TSharedPtr<FJsonObject>& P, FString& Error, TSharedPtr<FJsonObject>* OutNormalization)
{
    ...
    for (UObject* Asset : Assets)
    {
        ...
        if (!Mesh) continue;
        if (++MeshCount > 1 || Mesh->GetNumLODs() != 1 || Mesh->GetNumTriangles(0) > StaticPropNumber(P, TEXT("maximumTriangles")))
            return StaticPropFail(Error, TEXT("HarborProp requires one static mesh LOD within its triangle limit."));
        FMeshDescription* Description = Mesh->GetMeshDescription(0);
        if (!Description) return StaticPropFail(Error, TEXT("Imported prop lacks editable mesh geometry."));
        ...
        for (FVertexID Vertex : Description->Vertices().GetElementIDs())
        {
            ...
            if (Position.ContainsNaN() || Position.GetAbsMax() > 100000)
                return StaticPropFail(Error, TEXT("Imported prop contains invalid geometry coordinates."));
            Bounds += Position;
        }
        const FVector Size = Bounds.GetSize();
        if (!Bounds.IsValid || Size.GetMin() < 0.01)
            return StaticPropFail(Error, TEXT("Harbor props must have nonzero extent on every axis."));
        ...
        const double Scale = StaticPropNumber(P, TEXT("heightCm")) / Size.Z;
    ...
    for (UObject* Asset : Assets)
    {
        if (const UStaticMesh* Mesh = Cast<UStaticMesh>(Asset))
        {
            ...
            if (Mesh->GetNumLODs() != 1 || Mesh->GetNumTriangles(0) < 1 ||
                Mesh->GetNumTriangles(0) > StaticPropNumber(P, TEXT("maximumTriangles")) ||
                Mesh->GetStaticMaterials().Num() > StaticPropNumber(P, TEXT("maximumMaterials")) ||
                !B.IsValid || S.ContainsNaN() || S.GetMin() <= 0 || S.GetMax() > 10000 ||
                FMath::Abs(S.Z - StaticPropNumber(P, TEXT("heightCm"))) > 0.1 ||
                FMath::Abs(C.X) > 0.1 || FMath::Abs(C.Y) > 0.1 || FMath::Abs(B.Min.Z) > 0.1)
                return StaticPropFail(Error, TEXT("Static prop failed centimeter dimensions, bottom-center pivot or geometry/material limits."));
            const UBodySetup* Body = Mesh->GetBodySetup();
            if (!Body || Body->AggGeom.GetElementCount() != 1 || Body->AggGeom.BoxElems.Num() != 1 ||
                Body->CollisionTraceFlag != CTF_UseSimpleAndComplex)
                return StaticPropFail(Error, TEXT("Static prop requires one simple box collision shape."));
            ...
            if (!Box.Center.Equals(C, 0.1) || !FVector(Box.X, Box.Y, Box.Z).Equals(S, 0.1) || !Box.Rotation.IsNearlyZero())
                return StaticPropFail(Error, TEXT("Simple collision no longer matches the prop bounds."));
        }
        ...
        else if (const UTexture2D* Texture = Cast<UTexture2D>(Asset))
        {
            ...
            if (!Texture->Source.IsValid() || Texture->Source.GetSizeX() < 1 || Texture->Source.GetSizeY() < 1 ||
                Texture->Source.GetSizeX() > StaticPropNumber(P, TEXT("maximumTextureSize")) ||
                Texture->Source.GetSizeY() > StaticPropNumber(P, TEXT("maximumTextureSize")))
                return StaticPropFail(Error, TEXT("Static prop texture failed native decode or dimensions budget."));
        }
        else return StaticPropFail(Error, TEXT("HarborProp bundle contains an unsupported asset class."));
    }
    if (MeshCount != 1 || MaterialCount > StaticPropNumber(P, TEXT("maximumMaterials")) ||
        TextureCount > StaticPropNumber(P, TEXT("maximumMaterials")) * 4)
        return StaticPropFail(Error, TEXT("HarborProp bundle exceeds mesh/material/texture count limits."));
    return true;
}
bool FHansaStaticProp::RenderPreview(UStaticMesh* Mesh, const FString& Filename, UTexture2D*& Texture, FString& Error)
{
    Texture = nullptr;
    if (!Mesh || !FApp::CanEverRender()) return StaticPropFail(Error, TEXT("A rendering-enabled Editor is required for the prop review scene."));
    FAssetCompilingManager::Get().FinishAllCompilation();
    ...
    if (!Target->GameThread_GetRenderTargetResource()->ReadPixels(Pixels) || Pixels.Num() != 1280 * 720)
        return StaticPropFail(Error, TEXT("Could not read deterministic prop preview pixels."));
    ...
    if (!FFileHelper::SaveArrayToFile(Png, *Filename))
        return StaticPropFail(Error, TEXT("Could not retain the native 1280x720 review capture."));
