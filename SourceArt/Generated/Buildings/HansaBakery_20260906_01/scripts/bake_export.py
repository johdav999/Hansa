...
baked={};inventory=[]
for m in mats:
 print('BAKING',m.name,flush=True)
 work=m.copy();plane.data.materials.clear();plane.data.materials.append(work)
 nodes=work.node_tree.nodes;links=work.node_tree.links;bs=nodes.get('Principled BSDF');output=nodes.get('Material Output');em=nodes.new('ShaderNodeEmission')
 ...
 for kind in ['BaseColor','Roughness','Normal']:
  im=bpy.data.images.new(m.name+'_'+kind,width=1024,height=1024,alpha=False)
  im.colorspace_settings.name='sRGB' if kind=='BaseColor' else 'Non-Color';image_node.image=im;nodes.active=image_node
  for l in list(output.inputs['Surface'].links):links.remove(l)
  if kind=='Normal':
   links.new(bs.outputs[0],output.inputs['Surface']);bpy.ops.object.bake(type='NORMAL',normal_space='TANGENT')
  else:
   for l in list(em.inputs['Color'].links):links.remove(l)
   if kind=='BaseColor' and original_color:links.new(original_color,em.inputs['Color'])
   elif kind=='BaseColor':em.inputs['Color'].default_value=bs.inputs['Base Color'].default_value
   else:r=bs.inputs['Roughness'].default_value;em.inputs['Color'].default_value=(r,r,r,1)
   links.new(em.outputs[0],output.inputs['Surface']);bpy.ops.object.bake(type='EMIT')
  final=P/'textures'/(m.name+'_'+kind+'.png');tmp=final.with_suffix('.tmp.png');im.filepath_raw=str(tmp);im.file_format='PNG';im.save()
  check=bpy.data.images.load(str(tmp),check_existing=False);assert tuple(check.size)==(1024,1024);assert len(check.pixels)>0;bpy.data.images.remove(check)
  tmp.replace(final);im.filepath_raw=str(final);maps[kind]=str(final)
  bpy.data.images.remove(im)
 baked[m.name]=maps
 inventory.append({'name':m.name,'maps':maps,'native_dimensions':[1024,1024],'extent_m':[2,2],'nominal_density_px_m':512,'normal_convention':'OpenGL +Y','source':'Original procedural; no photo pixels','metallic':bs.inputs['Metallic'].default_value,'roughness':bs.inputs['Roughness'].default_value,'color_space':{'BaseColor':'sRGB','Roughness':'linear','Normal':'linear'}})
