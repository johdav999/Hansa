...
P=pathlib.Path(__file__).resolve().parents[1];bpy.ops.wm.open_mainfile(filepath=str(P/'checkpoints/portable_1024.blend'));S=bpy.context.scene;o=bpy.data.objects['SM_HansaMill'];uv=o.data.uv_layers.active.data;o.data.calc_loop_triangles();stats={m.name:[] for m in o.data.materials}
...
light=bpy.data.objects['Large soft daylight'];light.hide_render=False;light.location=(-1,-2,4);light.rotation_euler=(Vector((0,0,0))-light.location).to_track_quat('-Z','Y').to_euler();light.data.energy=150;light.data.size=3
...
for name in ['Oak','RoofTimber','Limestone','ForgedIron']:
