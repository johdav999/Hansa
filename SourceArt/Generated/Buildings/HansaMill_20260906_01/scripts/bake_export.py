...
bpy.ops.wm.open_mainfile(filepath=str(P/'checkpoints/mill_r3.blend'))
S=bpy.context.scene
names=['Oak','RoofTimber','Limestone','ForgedIron','ShelteredOak'];materials=[bpy.data.materials[n] for n in names]
...
for m in materials:
 ...
 rec={'name':m.name,'maps':{},'source':m.node_tree.nodes.get('ImageGen_native_color').image.filepath if m.node_tree.nodes.get('ImageGen_native_color') else 'Procedural iron: no image detail needed','uv_coverage_m':1,'normal_convention':'OpenGL +Y','base_color_px_per_m':1254,'physical_px_per_m':1024,'weathering':'Weathering vertex colors multiplied separately'}
 for kind,size in [('BaseColor',1254),('Roughness',1024),('Normal',1024)]:
...
for rec in inventory:
 m=bpy.data.materials[rec['name']];n=m.node_tree.nodes;l=m.node_tree.links;n.clear();b=n.new('ShaderNodeBsdfPrincipled');out=n.new('ShaderNodeOutputMaterial');l.new(b.outputs[0],out.inputs[0]);b.inputs['Metallic'].default_value=.62 if rec['name']=='ForgedIron' else 0
...
bpy.context.view_layer.objects.active=asset[0];bpy.ops.object.join();combined=bpy.context.object;combined.name='SM_HansaMill';S.cursor.location=(0,0,0);bpy.ops.object.origin_set(type='ORIGIN_CURSOR');bpy.ops.object.transform_apply(location=False,rotation=True,scale=True)
...
bpy.ops.export_scene.gltf(filepath=str(P/'exports/HansaMill.glb'),export_format='GLB',use_selection=True,export_apply=True,export_colors=True)
bpy.ops.export_scene.fbx(filepath=str(P/'exports/HansaMill.fbx'),use_selection=True,object_types={'MESH'},axis_forward='-Z',axis_up='Y',apply_unit_scale=True,path_mode='COPY',embed_textures=False,use_mesh_modifiers=True,add_leaf_bones=False)
