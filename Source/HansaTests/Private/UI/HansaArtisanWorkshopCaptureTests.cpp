...
namespace {
class FArtisanWorkshopCapture final:public IAutomationLatentCommand {
public:
 explicit FArtisanWorkshopCapture(FAutomationTestBase* T):Test(T),Started(FPlatformTime::Seconds()){}
 bool Update() override {
  ...
  if(!Placed){
   ...
   for(const TCHAR* Id:{TEXT("Building.CharcoalBurner"),TEXT("Building.Smithy"),TEXT("Building.Tannery"),TEXT("Building.Shoemaker")})
  ...
  if(!Selected){
   ...
   for(TActorIterator<AHansaBuildingWorldProjectionActor> It(W);It;++It)if(It->GetBuildingDefinitionId()==Id){
    C->OnWorldSelectionChanged.Broadcast(*It,FHitResult());
    {
     ...
     Test->TestTrue(TEXT("Live workshop uses its production mesh"),MeshFound);
    }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FArtisanWorkshopViewport,"Hansa.UI.ArtisanProduction.RealWorkshops",EAutomationTestFlags::ClientContext|EAutomationTestFlags::EngineFilter|EAutomationTestFlags::NonNullRHI)
bool FArtisanWorkshopViewport::RunTest(const FString&){ADD_LATENT_AUTOMATION_COMMAND(FArtisanWorkshopCapture(this));return true;}
#endif
