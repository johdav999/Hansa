...
$artifacts=New-HansaArtifactDirectory -Context $context -Operation "residence-inspector-$Width-$Height"
...
$arguments=@($context.ProjectFile,'/Game/Hansa/World/Cities/Lubeck/L_Lubeck_MVP',
    ...
    '-ExecCmds=Automation RunTests Hansa.UI.ResidenceInspector.RealViewport',
...
try {
    ...
    Invoke-HansaNativeCommand -FilePath $context.UnrealEditorCommand -Arguments $arguments `
        -LogPath (Join-Path $artifacts 'Command.log') -FailureMessage 'Residence inspector capture failed.' | Out-Null
...
if($log -notmatch 'Test Completed\. Result=\{Success\} Name=\{RealViewport\}' -or $log -match 'Result=\{Fail') {
    throw "Residence inspector capture did not pass: $unrealLog"
}
Write-Output "Residence inspector capture passed: $artifacts"
