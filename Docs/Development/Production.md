...
`FHansaProductionFixture::TryCreate()` constructs the reviewed actor-free `mvp_production_chains_v1` fixture: the ten-good, eight-recipe, fourteen-building, six-need, two-tier and four-city-market automation registry; eight production building instances; one city inventory; and deterministic background salt supply. Fixture version `3` retains its immutable evidence hash `B0481C9F740D6C18`. The playable runtime catalog has a separate reviewed hash owned by `FHansaLubeckScenarioInitializer`; an editor reload test and runtime startup both fail closed if the cooked authored assets diverge from that value. Its fingerprints track the current version-13 state-hash contract, including automatic city-workforce participation.
