...
$artifactDirectory = New-HansaArtifactDirectory -Context $context -Operation "p23-native-$Width-$Height"
$unrealLog = Join-Path $artifactDirectory 'Unreal.log'
New-Item -ItemType Directory -Force (Join-Path $context.ProjectRoot 'Saved/P23') | Out-Null
$arguments = @($context.ProjectFile, '/Game/Hansa/World/Cities/Lubeck/L_Lubeck_MVP',
    ...
    '-ExecCmds=Automation RunTests Hansa.UI.HudPolish.RealViewport',
...
try {
    ...
    Invoke-HansaNativeCommand -FilePath (Join-Path (Split-Path $context.UnrealEditorCommand) 'UnrealEditor-Win64-DebugGame-Cmd.exe') -Arguments $arguments `
        -LogPath (Join-Path $artifactDirectory 'Command.log') -FailureMessage 'P23 native capture failed.' | Out-Null
} finally {
...
if ($log -notmatch 'Test Completed\. Result=\{Success\} Name=\{RealViewport\}' -or $log -match 'Result=\{Fail' -or
    $log -notmatch 'Found 1 automation tests') { throw "P23 capture did not pass: $unrealLog" }
Write-Output "P23 native capture passed: $artifactDirectory"
