...
for m in list(bpy.data.materials):
 ...
 for c,target in [('BaseColor','Base Color'),('Roughness','Roughness'),('NormalGL','Normal')]:
  t=em.node_tree.nodes.new('ShaderNodeTexImage');t.image=bpy.data.images.load(maps[c],check_existing=True);t.image.colorspace_settings.name='sRGB' if c=='BaseColor' else 'Non-Color'
 ...
 export_mats[m.name]=em;records.append({'material':m.name,'portable_material':em.name,'maps':maps,'extent_m':2.5,'baked_density_px_m':614.4,'source':m.get('source','procedural'),'normal_convention':'OpenGL +Y','metallic':m.get('metallic',0)})
