...
$artifactDirectory = New-HansaArtifactDirectory -Context $context -Operation "p24-native-$Width-$Height"
$unrealLog = Join-Path $artifactDirectory 'Unreal.log'
New-Item -ItemType Directory -Force (Join-Path $context.ProjectRoot 'Saved/P24') | Out-Null
$arguments = @($context.ProjectFile, '/Game/Hansa/World/Cities/Lubeck/L_Lubeck_MVP',
    ...
    '-ExecCmds=Automation RunTests Hansa.UI.CityOverviewPolish.RealViewport',
...
try {
    ...
    Invoke-HansaNativeCommand -FilePath (Join-Path (Split-Path $context.UnrealEditorCommand) 'UnrealEditor-Win64-DebugGame-Cmd.exe') -Arguments $arguments `
        -LogPath (Join-Path $artifactDirectory 'Command.log') -FailureMessage 'P24 native capture failed.' | Out-Null
} finally {
...
if ($log -notmatch 'Test Completed\. Result=\{Success\} Name=\{RealViewport\}' -or $log -match 'Result=\{Fail' -or
    $log -notmatch 'Found 1 automation tests') { throw "P24 capture did not pass: $unrealLog" }
Write-Output "P24 native capture passed: $artifactDirectory"
