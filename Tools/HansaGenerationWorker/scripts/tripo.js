...
if (["estimate","submit"].includes(action) && target) {
  ...
  if(request.providerId!=="tripo"||request.intendedAssetRole!=="HarborProp")throw new Error("This CLI accepts only Tripo HarborProp requests.");
  ...
  result=await client.call({status:"job.get",cancel:"job.cancel",retain:"job.media.retain"}[action],{jobId:target,...(action==="retain"?{outputIndex:0}:{})});
  ...
  throw new Error("Usage: tripo.js estimate|submit <request.json> [--approve-spend=Reviewer --confirm-cost=N], status|cancel|retain <job-id>, or retry <job-id> --approve-spend=Reviewer --confirm-budget=N --retry-key=Unique.");
}
